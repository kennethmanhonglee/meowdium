document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (userCommentForm) {
    userCommentForm.addEventListener("submit", async (e) => {
      ...
      try {
        const res = await fetch(`${window.location.href}/pawments`, {
          // TODO: change to API once functioning
          method: "POST",
        ...
        let { id, userId, content, userName, updatedAt } = await res.json();
  ...
  for (let ele of pawmentsList.childNodes) {
    ...
    if (ele.childNodes.length === 3) {
      ...
      const pawmentDeleteButton = document.querySelector(
        `.delete-button-${pawmentId}`
      );
      pawmentDeleteButton.addEventListener("click", async (e) => {
        ...
        // dynamically remove comment
        const pawmentDivToDelete = document.querySelector(
          `.pawment-${pawmentId}`
        );
        pawmentDivToDelete.remove();
      });

      const pawmentEditButton = document.querySelector(
        `.edit-button-${pawmentId}`
      );
      pawmentEditButton.addEventListener("click", async (e) => {
        ...
        const editSubmitButton = document.querySelector(
          `.edit-submit-button-${pawmentId}`
        );
        const editCancelButton = document.querySelector(
          `.edit-cancel-button-${pawmentId}`
        );
        editSubmitButton.removeAttribute("hidden");
        editCancelButton.removeAttribute("hidden");
        pawmentEditButton.setAttribute("hidden", "true");
